...
struct gp2a_data {
	...
	struct device *dev;
	struct device *prox_factory_device;
	struct device *light_factory_device;
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (shutdown == true) {
		pr_info("%s, shutdown true.", __func__);
		goto done;
	}
...
static int gp2a_prox_open_calibration(struct gp2a_data *data)
{
	...
	cal_filp = filp_open(CAL_PATH, O_RDONLY, S_IRUGO | S_IWUSR | S_IWGRP);
...
static void gp2a_iio_prox_enable(struct iio_dev *indio_dev, int enable)
{
	...
	if (enable) {
		if (data->prox_enabled == SENSOR_DISABLE) {
			...
			/* data update */
			value = (int32_t *) kmalloc(indio_dev->scan_bytes,
							GFP_KERNEL);
			if (value == NULL)
				goto done;
			...
			if (indio_dev->scan_timestamp)
				*(s64 *)((u8 *)value + ALIGN(len, sizeof(s64)))
					= 0;
	...
	struct gp2a_data *data)
{
	...
	struct gp2a_data *st = iio_priv(indio_dev);
	indio_dev->pollfunc =
		iio_alloc_pollfunc(&gp2a_iio_pollfunc_store_boottime,
...
static void gp2a_light_iio_destroy(struct gp2a_data *data)
{
	iio_device_unregister(data->indio_dev_light);
	gp2a_prox_remove_trigger(data->indio_dev_light);
	gp2a_prox_remove_buffer(data->indio_dev_light);
	iio_device_free(data->indio_dev_light);
}

static int gp2a_light_iio_probe(struct gp2a_data *data,
	struct i2c_client *client)
{
	int err;
	struct iio_dev *indio_light;
	struct gp2a_data *data_light;

	pr_info("%s, start\n", __func__);

	indio_light = iio_device_alloc(sizeof(*data_light));
	if (indio_light == NULL) {
		pr_err("%s, iio device alloc failed\n", __func__);
		err = -ENOMEM;
		goto error_light_probe_failed;
	}

	i2c_set_clientdata(client, indio_light);

	indio_light->name = GP2A_LIGHT_NAME;
	indio_light->dev.parent = &client->dev;
	indio_light->info = &gp2a_light_info;
	indio_light->channels = gp2a_light_channels;
	indio_light->num_channels = ARRAY_SIZE(gp2a_light_channels);
	indio_light->modes = INDIO_DIRECT_MODE;

	data_light = iio_priv(indio_light);
	data_light->client = client;
	data_light->indio_dev_light = indio_light;

	mutex_init(&data_light->lock);
	mutex_init(&data_light->data_mutex);
	mutex_init(&data_light->light_mutex);
	mutex_init(&data_light->prox_mutex);
	spin_lock_init(&data_light->spin_lock);

	err = gp2a_light_probe_buffer(indio_light);
	if (err) {
		pr_err("%s, light probe buffer failed\n", __func__);
		goto error_free_light_dev;
	}

	err = gp2a_light_probe_trigger(indio_light, data_light);
	if (err) {
		pr_err("%s, light probe trigger failed\n", __func__);
		goto error_remove_light_buffer;
	}

	err = iio_device_register(indio_light);
	if (err) {
		pr_err("%s, light iio register failed\n", __func__);
		goto error_remove_light_trigger;
	}

	INIT_DELAYED_WORK(&data_light->light_work, gp2a_work_func_light);

	data_light->sampling_frequency_light = 5;
	data_light->light_delay =
		MSEC_PER_SEC / data_light->sampling_frequency_light;

	data->indio_dev_light = indio_light;
...
error_remove_light_trigger:
	gp2a_light_remove_trigger(indio_light);
error_remove_light_buffer:
	gp2a_light_remove_buffer(indio_light);
error_free_light_dev:
	iio_device_free(indio_light);
error_light_probe_failed:
	return err;
}

static int gp2a_prox_iio_probe(struct gp2a_data *data,
	struct i2c_client *client, int irq_gpio)
{
	int err;
	struct iio_dev *indio_prox;
	struct gp2a_data *data_prox;
	pr_info("%s, start\n", __func__);

	indio_prox = iio_device_alloc(sizeof(*data_prox));
	if (indio_prox == NULL) {
		pr_err("%s, iio device alloc failed\n", __func__);
		err = -ENOMEM;
		goto error_prox_probe_failed;
	}

	i2c_set_clientdata(client, indio_prox);

	indio_prox->name = GP2A_PROX_NAME;
	indio_prox->dev.parent = &client->dev;
	indio_prox->info = &gp2a_prox_info;
	indio_prox->channels = gp2a_prox_channels;
	indio_prox->num_channels = ARRAY_SIZE(gp2a_prox_channels);
	indio_prox->modes = INDIO_DIRECT_MODE;

	data_prox = iio_priv(indio_prox);
	data_prox->client = client;
	data_prox->indio_dev_prox = indio_prox;

	mutex_init(&data_prox->lock);
	mutex_init(&data_prox->data_mutex);
	mutex_init(&data_prox->light_mutex);
	mutex_init(&data_prox->prox_mutex);

	wake_lock_init(&data_prox->prx_wake_lock, WAKE_LOCK_SUSPEND,
		"prx_wake_lock");
	spin_lock_init(&data_prox->spin_lock);

	err = gp2a_prox_probe_buffer(indio_prox);
	if (err) {
		pr_err("%s, prox probe buffer failed\n", __func__);
		goto error_free_prox_dev;
	}

	err = gp2a_prox_probe_trigger(indio_prox, data_prox);
	if (err) {
		pr_err("%s, prox probe trigger failed\n", __func__);
		goto error_remove_prox_buffer;
	}

	err = iio_device_register(indio_prox);
	if (err) {
		pr_err("%s, prox iio register failed\n", __func__);
		goto error_remove_prox_trigger;
	}

	data_prox->gpio = irq_gpio;

	err = gp2a_setup_irq(data_prox);
	if (err) {
		pr_err("%s: could not setup irq\n", __func__);
		goto error_setup_irq;
	}

	INIT_WORK(&data_prox->proximity_work, gp2a_work_func_iio_prox);

	data_prox->sampling_frequency_prox = 5;
	data_prox->prox_delay =
		MSEC_PER_SEC / data_prox->sampling_frequency_prox;

	data->indio_dev_prox = indio_prox;
...
error_setup_irq:
	iio_device_unregister(indio_prox);
error_remove_prox_trigger:
	gp2a_prox_remove_trigger(indio_prox);
error_remove_prox_buffer:
	gp2a_prox_remove_buffer(indio_prox);
error_free_prox_dev:
	iio_device_free(indio_prox);
error_prox_probe_failed:
	return err;
}
	...
	const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		...
		goto error_i2c_check_functionality;
	}

	/* for factory device struct */
	data = kzalloc(sizeof(*data), GFP_KERNEL);
	if (!data) {
		pr_err("%s, kmalloc failed\n", __func__);
		err = -ENOMEM;
		goto error_kmalloc_fail;
	}
	...
	/* factory light sensor */
	err = sensors_register(data->light_factory_device,
			data, light_sensor_attrs, "light_sensor");
	if (err) {
		pr_err("%s: cound not register prox sensor device(%d).\n",
			__func__, err);
		goto error_register_light_factory;
	}

	/* factory prox sensor */
	err = sensors_register(data->prox_factory_device,
			data, prox_sensor_attrs, "proximity_sensor");
	if (err) {
		pr_err("%s: cound not register prox sensor device(%d).\n",
			__func__, err);
		goto error_register_prox_factory;
	}

	err = gp2a_light_iio_probe(data, client);
	if (err < 0) {
		pr_err("%s, light iio_probe failed\n", __func__);
		goto error_iio_light_probe;
	}

	err = gp2a_prox_iio_probe(data, client, data->gpio);
	if (err < 0) {
		pr_err("%s, prox iio_probe failed\n", __func__);
		goto error_iio_prox_probe;
	}
...
error_iio_prox_probe:
	gp2a_light_iio_destroy(data);
error_iio_light_probe:
	sensors_unregister(data->prox_factory_device, prox_sensor_attrs);
error_register_prox_factory:
	sensors_unregister(data->light_factory_device, light_sensor_attrs);
error_register_light_factory:
error_i2c_fail:
error_gp2a_parse_dt:
	gpio_free(data->power_en);
	gpio_free(data->gpio);
	kfree(data);
error_kmalloc_fail:
error_i2c_check_functionality:
	pr_info("%s, failed %d\n", __func__, err);
	return err;
}
