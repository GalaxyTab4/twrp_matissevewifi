...
//bringup [
	msleep(200);
	ret = regulator_is_enabled(info->dtdata->vreg_vio);
	dev_err(&client->dev, "%s is enable %d\n", __func__, ret);

	ret = regulator_get_voltage(info->dtdata->vreg_vio);
	dev_err(&client->dev, "%s voltage %d\n", __func__, ret);

	ret = gpio_get_value(info->dtdata->gpio_vdd);
	dev_err(&client->dev, "%s ldo en %d\n", __func__, ret);
//bringup ]
