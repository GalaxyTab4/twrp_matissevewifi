			    ...
			    struct msmfb_overlay_data *req)
{
	...
	reg_val_axi = readl_relaxed(mdata->vbif_base + MMSS_VBIF_AXI_HALT_CTRL1);
	if(!(reg_val & BIT(18)))
		pr_err("ROT: DMA Pipe Fetch In Progress\n");

	MDSS_XLOG(reg_val, reg_val_axi);

	reg_val = readl_relaxed(mdata->vbif_base +
		MMSS_VBIF_XIN_HALT_CTRL0);
	writel_relaxed(reg_val & ~BIT(18),
		mdata->vbif_base + MMSS_VBIF_XIN_HALT_CTRL0);
	wmb();
	mutex_unlock(&mdata->reg_lock);

	MDSS_XLOG(rot->src_buf.p[0].addr, rot->src_buf.p[1].addr,rot->dst_buf.p[0].addr, rot->dst_buf.p[1].addr);
