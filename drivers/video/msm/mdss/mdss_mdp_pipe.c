...
void mdss_mdp_pipe_debug_dump(struct mdss_mdp_pipe *pipe, char *pipe_name)
{
	struct mdss_data_type *mdata = NULL;

	if (!pipe)
		pr_err("pipe is invalid\n");

	if (pipe->mixer_left && pipe->mixer_left->ctl)
		mdata = pipe->mixer_left->ctl->mdata;

	pr_err("******** %s pipe %d dump start ********\n", pipe_name, pipe->num);

	pr_err("ply_cnt=0x%08x flags=0x%x bwc=%d width=%d height=%d h_dec=%d v_dec=%d\n",
		pipe->play_cnt, pipe->flags, pipe->bwc_mode,
		pipe->img_width, pipe->img_height, pipe->horz_deci, pipe->vert_deci);

	pr_err("mixer_stage=%d mixer_num=%d mixer_ref_cnt=%d rot=%d params_changed=%d\n",
		pipe->mixer_stage, pipe->mixer_left->num, pipe->mixer_left->ref_cnt,
		pipe->mixer_left->rotator_mode, pipe->params_changed);
	...
	if (mdata) {
		...
		for (i = 0; i < mdata->nctl; i++) {
			...
			if (ctl && ctl->power_on) {
				u32 lm0, lm1, lm2, lm3, lm4;
				lm0 = readl_relaxed(ctl->base);
				lm1 = readl_relaxed(ctl->base + 0x04);
				lm2 = readl_relaxed(ctl->base + 0x08);
				lm3 = readl_relaxed(ctl->base + 0x0C);
				lm4 = readl_relaxed(ctl->base + 0x10);
				pr_err("ctl_num=%d vsync_cnt=0x%x play_cnt=0x%x lm0=0x%x lm1=0x%x lm2=0x%x lm3=0x%x lm4=0x%x\n",
					ctl->num, ctl->vsync_cnt, ctl->play_cnt, lm0, lm1, lm2, lm3, lm4);
			}
		}

		for (i = 0; i < mdata->nvig_pipes; i++) {
			struct mdss_mdp_pipe *pipe = mdata->vig_pipes + i;
			if (pipe) {
				u32 src0, src1, src2, src3;
				src0 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC0_ADDR);
				src1 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC1_ADDR);
				src2 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC2_ADDR);
				src3 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC3_ADDR);
				pr_err("Vig pipe=%d current src0=0x%x src1=0x%x src2=0x%x src3=0x%x\n",
					i, src0, src1, src2, src3);
			}
		}
		for (i = 0; i < mdata->nrgb_pipes; i++) {
			struct mdss_mdp_pipe *pipe = mdata->rgb_pipes + i;
			if (pipe) {
				u32 src0, src1, src2, src3;
				src0 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC0_ADDR);
				src1 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC1_ADDR);
				src2 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC2_ADDR);
				src3 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC3_ADDR);
				pr_err("rgb pipe=%d current src0=0x%x src1=0x%x src2=0x%x src3=0x%x\n",
					i, src0, src1, src2, src3);
			}
		}
		for (i = 0; i < mdata->ndma_pipes; i++) {
			struct mdss_mdp_pipe *pipe = mdata->dma_pipes + i;
			if (pipe) {
				u32 src0, src1, src2, src3;
				src0 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC0_ADDR);
				src1 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC1_ADDR);
				src2 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC2_ADDR);
				src3 = readl_relaxed(pipe->base + MDSS_MDP_REG_SSPP_CURRENT_SRC3_ADDR);
				pr_err("dma pipe=%d current src0=0x%x src1=0x%x src2=0x%x src3=0x%x\n",
					i, src0, src1, src2, src3);
			}
		}
	}
	...
	//dss_reg_dump(pipe->base, 0x12C, "MDP iommu page fault dump:", true);

	pr_err("******** %s pipe %d dump end ********\n", pipe_name, pipe->num);
}
	...
	bool ignore_force_on)
{
	...
	reg_clk_status = readl_relaxed(mdata->mdp_base + MDP_CLK_STATUS0);

	pr_debug("pipe#:%d MDP_CLK_STATUS0: 0x%x\n", pipe->num, reg_clk_status);
...
int mdss_mdp_pipe_fetch_halt(struct mdss_mdp_pipe *pipe)
{
	...
	if (!is_idle && !in_use) {
		...
		pr_debug("%pS: pipe%d is not idle. xin_id=%d\n",
