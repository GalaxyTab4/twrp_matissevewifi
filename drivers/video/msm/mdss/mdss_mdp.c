		...
		struct device *dev, unsigned long iova, int flags, void *token)
{
	...
	pr_err("MDP registers\n");
	mdss_dump_reg(mdata->mdss_base, 5157);

	pr_err("MDP Control0 Registers\n");
	mdss_dump_reg(mdata->mdss_base + 0x2000,49);

	pr_err("MDP Control1 Registers\n");
	mdss_dump_reg(mdata->mdss_base + 0x2200,49);

	pr_err("MDP Control2 Registers\n");
	mdss_dump_reg(mdata->mdss_base + 0x2400,49);

	pr_err("ViG0 SSPP SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x5000,297);

	pr_err("ViG0 SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x5200,373);

	pr_err("RGB0 SSPP SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x15000,297);

	pr_err("RGB1 SSPP SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x17000,297);

	pr_err("DMA0 SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x25000,297);

	pr_err("Display Layer Mixer0 Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x45000, 901);

	pr_err("Display Layer Mixer3 Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x48000, 901);

	pr_err("Display WB0 SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x65000, 689);

	pr_err("Display WB1 SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x65800, 689);

	pr_err("Display WB2 SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x66000, 689);

	pr_err("Display Interface SWI Register Set\n");
	mdss_dump_reg(mdata->mdss_base + 0x6B000, 1043);

	pr_err("Display VBIF registers\n");
	mdss_dump_reg(mdata->vbif_base, 8196);

	pr_err("Display debug registers\n");
	for (i = 54; i < 61; i++) {
		writel_relaxed(i<<4 | 0x1, mdata->mdss_base + 0x1298);
		pr_err("%d:1: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));

		writel_relaxed(i<<4 | 0x3, mdata->mdss_base + 0x1298);
		pr_err("%d:3: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));

		writel_relaxed(i<<4 | 0x5, mdata->mdss_base + 0x1298);
		pr_err("%d:5: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));

		writel_relaxed(i<<4 | 0x7, mdata->mdss_base + 0x1298);
		pr_err("%d:7: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));

		writel_relaxed(i<<4 | 0x9, mdata->mdss_base + 0x1298);
		pr_err("%d:9: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));

		writel_relaxed(i<<4 | 0xB, mdata->mdss_base + 0x1298);
		pr_err("%d:B: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));

		writel_relaxed(i<<4 | 0xD, mdata->mdss_base + 0x1298);
		pr_err("%d:D: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));

		writel_relaxed(i<<4 | 0xF, mdata->mdss_base + 0x1298);
		pr_err("%d:F: val(mdss_base + 0x1298) = 0x%x\n", i,
			readl_relaxed(mdata->mdss_base + 0x129C));
	}

	writel_relaxed(0x521, mdata->mdss_base + 0x1298);
	pr_err("521: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));

	writel_relaxed(0x523, mdata->mdss_base + 0x1298);
	pr_err("523: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));

	writel_relaxed(0x525, mdata->mdss_base + 0x1298);
	pr_err("525: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));

	writel_relaxed(0x527, mdata->mdss_base + 0x1298);
	pr_err("527: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));

	writel_relaxed(0x529, mdata->mdss_base + 0x1298);
	pr_err("529: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));

	writel_relaxed(0x52B, mdata->mdss_base + 0x1298);
	pr_err("52B: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));

	writel_relaxed(0x52D, mdata->mdss_base + 0x1298);
	pr_err("52D: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));

	writel_relaxed(0x52F, mdata->mdss_base + 0x1298);
	pr_err("52F: val(mdss_base + 0x1298) = 0x%x\n",
		readl_relaxed(mdata->mdss_base + 0x129C));
	...
	switch (fsynr1 & 0xF) {
	case 0x0:
		pr_err("page fault caused by vig0 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->vig_pipes[0], "vig");
		break;
	case 0x1:
		pr_err("page fault caused by rgb0 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->rgb_pipes[0], "rgb");
		break;
	case 0x2:
		pr_err("page fault caused by dma_r0 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->dma_pipes[0], "dma");
		BUG();
		break;
	case 0x3:
		pr_err("page fault caused by rot0_w pipe\n");
		break;
	case 0x4:
		pr_err("page fault caused by vig1 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->vig_pipes[1], "vig");
		break;
	case 0x5:
		pr_err("page fault caused by rgb1 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->rgb_pipes[1], "rgb");
		break;
	case 0x6:
		pr_err("page fault caused by dma_w1 pipe\n");
		break;
	case 0x7:
		pr_err("page fault caused by cursor pipe\n");
		break;
	case 0x8:
		pr_err("page fault caused by vig2 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->vig_pipes[2], "vig");
		break;
	case 0x9:
		pr_err("page fault caused by rgb2 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->rgb_pipes[2], "rgb");
		break;
	case 0xA:
		pr_err("page fault caused by dma_r1 pipe\n");
		mdss_mdp_pipe_debug_dump(&mdss_res->dma_pipes[1], "dma");
		break;
	case 0xB:
		pr_err("page fault caused by rot1_w pipe\n");
		break;
	default:
		pr_err("page fault caused by unkown pipe\n");
	}
