...
#if defined(CONFIG_FUELGAUGE_MAX77849_VOLTAGE_TRACKING)
	if (fg_i2c_read(client, VFSOC_REG, data, 2) < 0) {
#else
...
#if defined(CONFIG_FUELGAUGE_MAX77849_VOLTAGE_TRACKING)
static int max77849_set_temperature(struct i2c_client *client, int temperature)
{
	u8 data[2];
...
static void full_comp_work_handler(struct work_struct *work)
{
	struct sec_fuelgauge_info *fuelgauge =
		container_of(work, struct sec_fuelgauge_info, full_comp_work.work);
	int avg_current;
	union power_supply_propval value;
...
#if defined(CONFIG_FUELGAUGE_MAX77849_VOLTAGE_TRACKING)
			val->intval = fg_read_soc(fuelgauge->client);
#else
			    ...
			    const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TEMP_AMBIENT:
#if defined(CONFIG_FUELGAUGE_MAX77849_VOLTAGE_TRACKING)
		max77849_set_temperature(fuelgauge->client, val->intval);
#endif
	case POWER_SUPPLY_PROP_PRESENT:
		break;
	default:
