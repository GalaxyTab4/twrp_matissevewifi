...
#define SEC_FORCE_FULL_INTERVAL 120
#else
#define SEC_FORCE_FULL_INTERVAL 30
#endif
#define BATTERY_SWELLING_FLOAT_VOLTAGE 4250
...
void set_earjack_state(void)
{
	if (g_battery) {
		...
		pr_info("%s: EarJack (%d)\n", __func__, sec_jack_onoff());
	}
}

static void sec_bat_check_earjack(struct sec_battery_info *battery)
{
	...
	if (!battery->swelling_mode) {
		...
		if (battery->swelling_block_passed < battery->swelling_block_time) {
			...
			if (temperature <= battery->swelling_temp_high_recovery) {
				...
				val.intval = BATTERY_SWELLING_FLOAT_VOLTAGE;
				psy_do_property(battery->pdata->charger_name, set,
						POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
				battery->swelling_block = false;
				battery->charging_mode = SEC_BATTERY_CHARGING_1ST;
...
{
	union power_supply_propval value;
#if defined(CONFIG_AFC_CHARGER_MODE) || defined(CONFIG_CHARGER_SM5418)
	...
	if (battery->status != POWER_SUPPLY_STATUS_FULL) {
		battery->capacity = value.intval;
		   ...
		   ((c_ts.tv_sec - old_ts.tv_sec) >= SEC_FORCE_FULL_INTERVAL)) {
		battery->capacity++;
