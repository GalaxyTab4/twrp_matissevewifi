...
#if defined(CONFIG_SEC_E5_PROJECT) || defined(CONFIG_MACH_A5VE_CHN_OPEN)
...
#ifdef CONFIG_SEC_A5U_KOR_EARJACK_WR
enum {
	EARJACK_WR_MODE_NORMAL = 0,
	EARJACK_WR_MODE_500MA,
	EARJACK_WR_MODE_100MA
};
#endif
		...
		int onoff)
{
	...
	if ((onoff > 0) && (prev_charging_status == false)) {
		...
		if (charger->rev_id >= 6 || sldo_vbusdet) {
		...
		int current_limit)
{
	...
	|| defined(CONFIG_SEC_E7_PROJECT) || defined(CONFIG_SEC_SERRANOVELTE_PROJECT)  || defined(CONFIG_SEC_SERRANOVE_PROJECT) || defined(CONFIG_SEC_A5U_KOR_EARJACK_WR) 
...
#ifdef CONFIG_SEC_A5U_KOR_EARJACK_WR
static void rt5033_check_earjack_state(struct rt5033_charger_data *charger,
		int earjack_state)
{
	union power_supply_propval value;
	int input_current_limit = charger->pdata->charging_current_table
			[charger->cable_type].input_current_limit,
		aicr = charger->pdata->charging_current_table
			[POWER_SUPPLY_TYPE_BATTERY].input_current_limit;
	int earjack_wr_mode = charger->earjack_wr_mode,
		eoc = 0;
	...
	if (earjack_state) {
		...
		if (value.intval >= 90) {
			earjack_wr_mode = EARJACK_WR_MODE_100MA;
			input_current_limit = 100;
		} else if (value.intval >= 75) {
			earjack_wr_mode = EARJACK_WR_MODE_500MA;
			input_current_limit = 500;
		} else {
			earjack_wr_mode = EARJACK_WR_MODE_NORMAL;
		}

		aicr = rt5033_get_input_current_limit(charger->rt5033->i2c_client);
	} else if (earjack_wr_mode != EARJACK_WR_MODE_NORMAL) {
		earjack_wr_mode = EARJACK_WR_MODE_NORMAL;
	}

	if (earjack_wr_mode != charger->earjack_wr_mode ||
		aicr != input_current_limit) {
		pr_info("%s: earjack(%d), mode(%d), input(%d), aicr(%d)\n",
			__func__, earjack_state, earjack_wr_mode, input_current_limit, aicr);
		rt5033_set_input_current_limit(charger, input_current_limit);
		if (!earjack_state) {
			eoc = rt5033_get_current_eoc_setting(charger);
			rt5033_set_charging_current(charger, eoc, 1);
		}
		charger->earjack_wr_mode = earjack_wr_mode;
	}
}
#endif
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		pr_info("%s: earjack state(%d)\n", __func__, val->intval);
		if (charger->is_charging) {
			rt5033_check_earjack_state(charger, val->intval);
		} else {
			charger->earjack_wr_mode = EARJACK_WR_MODE_NORMAL;
		}
		break;
#endif
	default:
