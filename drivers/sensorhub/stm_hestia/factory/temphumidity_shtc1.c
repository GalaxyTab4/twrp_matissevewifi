...
#elif defined(CONFIG_SEC_HESTIA2_PROJECT)
#define MODEL_NAME	"SM-G739F"
#else
				...
				unsigned long arg)
{
	...
	switch (cmd) {
		...
		case IOCTL_READ_ADC_BATT_DATA:
			...
			if (unlikely(ret)) {
				pr_err("[SSP]THERM read bluk adc1 data err(%d)",
					ret);
				goto ioctl_error;
			}
			break;
		...
		default:
			pr_err("[SSP]THERM temp ioctl cmd err(%d)", cmd);
			ret = EINVAL;
			goto ioctl_error;
	}
...
static int get_cp_thm_value(struct ssp_data *data)
{
	...
	err = qpnp_vadc_read(ssp_vadc, LR_MUX7_HW_ID, &results);
		mutex_unlock(&data->cp_temp_adc_lock);
		if (err) {
		pr_err("[SSP]THERM %s : error reading chn %d, rc = %d\n",
			__func__, LR_MUX7_HW_ID, err);
			return err;
		}
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP]THERM %s - current temp = %d\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP]THERM %s - engine_ver = %s_%s\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SSP]THERM %s - engine_ver = %s, %s\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP]THERM %s - engine_ver2 = %s_%s\n",
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	pr_info("[SSP]THERM %s - engine_ver2 = %s, %s\n",
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP]THERM %s cp_thm = %dmV\n", __func__, adc);
exit:
	return sprintf(buf, "%d\n", adc);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	pr_info("[SSP]THERM %s cp_thm2 = %dmV\n", __func__, adc);
exit:
	return sprintf(buf, "%d\n", adc);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	adc = get_cp_thm_value(data);
	if (adc < 0) {
		pr_err("[SSP]THERM %s - reading adc failed.(%d)\n",
			__func__, adc);
		temp = adc;
	} else {
	...
	pr_info("[SSP]THERM %s cp_temperature(Celsius * 10) = %d\n",
		__func__, temp);
	return sprintf(buf, "%d\n", temp);
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	if (iRet != SUCCESS) {
		pr_err("[SSP]THERM: %s - Timeout!! %d\n", __func__, iRet);
		goto exit;
	}

	pr_info("[SSP]THERM : %s - Check_CRC : %d\n", __func__, chTempBuf);
...
void initialize_temphumidity_factorytest(struct ssp_data *data)
{
	...
	if (ret < 0) {
		pr_err("[SSP]THERM register temphumidity misc device err(%d)",
			ret);
	}
	...
	if (IS_ERR(ssp_vadc)) {
		...
		if (ret != -EPROBE_DEFER)
			pr_err("[SSP]THERM %s: Fail to get vadc %d\n", __func__,
				ret);
	}
