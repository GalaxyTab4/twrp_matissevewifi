...
static int ssp_push_12bytes_buffer(struct iio_dev *indio_dev,
				u64 t, int *q)
{
...
void convert_acc_data(s16 *value)
{
	if (*value > MAX_ACCEL_2G)
		*value = ((MAX_ACCEL_4G - *value)) * (-1);
}
...
{
	input_report_rel(data->meta_input_dev,
			REL_DIAL, s->meta_data.what);
	input_report_rel(data->meta_input_dev,
			REL_HWHEEL, s->meta_data.sensor + 1);
...
void report_mag_data(struct ssp_data *data, struct sensor_value *magdata)
{
	...
	if (arrTemp[0] == 1) {
		input_report_rel(data->mag_input_dev,
					REL_X, arrTemp[0] + 1);
		input_report_rel(data->mag_input_dev,
					REL_Y, arrTemp[1]);
		input_report_rel(data->mag_input_dev,
		...
		input_report_rel(data->mag_input_dev,
					REL_RZ, arrTemp[5]);
		input_report_rel(data->mag_input_dev,
					REL_HWHEEL, arrTemp[6]);
		input_report_rel(data->mag_input_dev,
					REL_DIAL, arrTemp[7]);
		mdelay(5);
		input_sync(data->mag_input_dev);
	} else
		pr_info("[SSP] %s, not initialised, val = %d", __func__,
			arrTemp[0]);
#else
...
void report_mag_uncaldata(struct ssp_data *data,
			struct sensor_value *magdata)
{
...
void report_rot_data(struct ssp_data *data, struct sensor_value *rotdata)
{
	...
	ssp_push_17bytes_buffer(data->rot_indio_dev,
				rotdata->timestamp,
				rot_buf);
}
...
int initialize_event_symlink(struct ssp_data *data)
{
	...
	if (ret < 0)
		goto ret_temp_humi_sysfs_create_link;
#endif
#ifdef CONFIG_SENSORS_SSP_UVIS25
	ret = sysfs_create_link(&data->sen_dev->kobj,
	...
	if (ret < 0)
		goto ret_uv_sysfs_create_link;
#endif
	ret = sysfs_create_link(&data->sen_dev->kobj,
		&data->mag_input_dev->dev.kobj,
		data->mag_input_dev->name);
	if (ret < 0)
		goto ret_mag_sysfs_create_link;

	ret = sysfs_create_link(&data->sen_dev->kobj,
		&data->uncal_mag_input_dev->dev.kobj,
		data->uncal_mag_input_dev->name);
	if (ret < 0)
		goto ret_uncal_mag_sysfs_create_link;

	ret = sysfs_create_link(&data->sen_dev->kobj,
		&data->sig_motion_input_dev->dev.kobj,
		data->sig_motion_input_dev->name);
	if (ret < 0)
		goto ret_sig_motion_sysfs_create_link;

	ret = sysfs_create_link(&data->sen_dev->kobj,
		&data->uncalib_gyro_input_dev->dev.kobj,
		data->uncalib_gyro_input_dev->name);
	if (ret < 0)
		goto ret_uncalib_gyro_sysfs_create_link;

	ret = sysfs_create_link(&data->sen_dev->kobj,
		&data->step_cnt_input_dev->dev.kobj,
		data->step_cnt_input_dev->name);
	if (ret < 0)
		goto ret_step_cnt_sysfs_create_link;

	ret = sysfs_create_link(&data->sen_dev->kobj,
		&data->meta_input_dev->dev.kobj,
		data->meta_input_dev->name);
	if (ret < 0)
		goto ret_meta_sysfs_create_link;

	return SUCCESS;
ret_meta_sysfs_create_link:
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->step_cnt_input_dev->dev.kobj,
		data->step_cnt_input_dev->name);
ret_step_cnt_sysfs_create_link:
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->uncalib_gyro_input_dev->dev.kobj,
		data->uncalib_gyro_input_dev->name);
ret_uncalib_gyro_sysfs_create_link:
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->sig_motion_input_dev->dev.kobj,
		data->sig_motion_input_dev->name);
ret_sig_motion_sysfs_create_link:
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->uncal_mag_input_dev->dev.kobj,
		data->uncal_mag_input_dev->name);
ret_uncal_mag_sysfs_create_link:
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->mag_input_dev->dev.kobj,
		data->mag_input_dev->name);
ret_mag_sysfs_create_link:
#ifdef CONFIG_SENSORS_SSP_UVIS25
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->uv_input_dev->dev.kobj,
		data->uv_input_dev->name);
ret_uv_sysfs_create_link:
#endif
#ifdef CONFIG_SENSORS_SSP_SHTC1
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->temp_humi_input_dev->dev.kobj,
		data->temp_humi_input_dev->name);
ret_temp_humi_sysfs_create_link:
#endif
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->prox_input_dev->dev.kobj,
		data->prox_input_dev->name);
ret_prox_sysfs_create_link:
	sysfs_delete_link(&data->sen_dev->kobj,
		&data->light_input_dev->dev.kobj,
		data->light_input_dev->name);
ret_light_sysfs_create_link:
...
int initialize_input_dev(struct ssp_data *data)
{
	...
	ret = ssp_iio_configure_ring(data->accel_indio_dev);
	if (ret) {
		pr_err("[SSP] %s configure ring buffer fail\n", __func__);
		goto out_free_accel;
	}

	ret = iio_buffer_register(data->accel_indio_dev,
				  data->accel_indio_dev->channels,
				  data->accel_indio_dev->num_channels);
	if (ret)
		goto out_unreg_ring_accel;

	ret = iio_device_register(data->accel_indio_dev);
	if (ret)
	...
	ret = ssp_iio_configure_ring(data->gyro_indio_dev);
	if (ret) {
		pr_err("[SSP] %s configure ring buffer fail\n", __func__);
		goto out_free_gyro;
	}

	ret = iio_buffer_register(data->gyro_indio_dev,
				  data->gyro_indio_dev->channels,
				  data->gyro_indio_dev->num_channels);
	if (ret)
		goto out_unreg_ring_gyro;

	ret = iio_device_register(data->gyro_indio_dev);
	if (ret)
	...
	ret = ssp_iio_configure_ring(data->game_rot_indio_dev);
	if (ret) {
		pr_err("[SSP] %s configure ring buffer fail\n", __func__);
		goto out_free_game_rot;
	}

	ret = iio_buffer_register(data->game_rot_indio_dev,
				  data->game_rot_indio_dev->channels,
				  data->game_rot_indio_dev->num_channels);
	if (ret)
		goto out_unreg_ring_game_rot;

	ret = iio_device_register(data->game_rot_indio_dev);
	if (ret)
	...
	ret = ssp_iio_configure_ring(data->rot_indio_dev);
	if (ret) {
		pr_err("[SSP] %s configure ring buffer fail\n", __func__);
		goto out_free_rot;
	}

	ret = iio_buffer_register(data->rot_indio_dev,
				  data->rot_indio_dev->channels,
				  data->rot_indio_dev->num_channels);
	if (ret)
		goto out_unreg_ring_rot;

	ret = iio_device_register(data->rot_indio_dev);
	if (ret)
	...
	ret = ssp_iio_configure_ring(data->step_det_indio_dev);
	if (ret) {
		pr_err("[SSP] %s configure ring buffer fail\n", __func__);
		goto out_free_step_det;
	}

	ret = iio_buffer_register(data->step_det_indio_dev,
				   data->step_det_indio_dev->channels,
				   data->step_det_indio_dev->num_channels);
	if (ret)
		goto out_unreg_ring_step_det;

	ret = iio_device_register(data->step_det_indio_dev);
	if (ret)
	...
	ret = ssp_iio_configure_ring(data->pressure_indio_dev);
	if (ret) {
		pr_err("[SSP] %s configure ring buffer fail\n", __func__);
		goto out_free_pressure;
	}

	ret = iio_buffer_register(data->pressure_indio_dev,
				  data->pressure_indio_dev->channels,
				  data->pressure_indio_dev->num_channels);
	if (ret)
		goto out_unreg_ring_pressure;

	ret = iio_device_register(data->pressure_indio_dev);
	if (ret)
...
#ifdef CONFIG_SENSORS_SSP_UVIS25
	ret = input_register_device(uv_input_dev);
	if (ret < 0) {
		...
		goto ret_uv_input_unreg_device;
	}
#endif

	ret = input_register_device(light_input_dev);
	if (ret < 0) {
		input_free_device(light_input_dev);
		input_free_device(prox_input_dev);
#ifdef CONFIG_SENSORS_SSP_SHTC1
		...
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_light_input_unreg_device;
	}

	ret = input_register_device(prox_input_dev);
	if (ret < 0) {
		input_free_device(prox_input_dev);
#ifdef CONFIG_SENSORS_SSP_SHTC1
		input_free_device(temp_humi_input_dev);
		...
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_proximity_input_unreg_device;
	}

#ifdef CONFIG_SENSORS_SSP_SHTC1
	ret = input_register_device(temp_humi_input_dev);
	if (ret < 0) {
		input_free_device(temp_humi_input_dev);
		input_free_device(mag_input_dev);
		input_free_device(uncal_mag_input_dev);
		...
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_tmep_humi_input_unreg_device;
	}
#endif

	ret = input_register_device(mag_input_dev);
	if (ret < 0) {
		input_free_device(mag_input_dev);
		input_free_device(uncal_mag_input_dev);
		input_free_device(sig_motion_input_dev);
		...
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_mag_input_unreg_device;
	}

	ret = input_register_device(uncal_mag_input_dev);
	if (ret < 0) {
		input_free_device(uncal_mag_input_dev);
		input_free_device(sig_motion_input_dev);
		input_free_device(uncalib_gyro_input_dev);
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_uncal_mag_input_unreg_device;
	}

	ret = input_register_device(sig_motion_input_dev);
	if (ret < 0) {
		input_free_device(sig_motion_input_dev);
		input_free_device(uncalib_gyro_input_dev);
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_sig_motion_input_unreg_device;
	}

	ret = input_register_device(uncalib_gyro_input_dev);
	if (ret < 0) {
		input_free_device(uncalib_gyro_input_dev);
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_uncalib_gyro_input_unreg_device;
	}

	ret = input_register_device(step_cnt_input_dev);
	if (ret < 0) {
		input_free_device(step_cnt_input_dev);
		input_free_device(meta_input_dev);
		goto ret_step_cnt_input_unreg_device;
	}

	ret = input_register_device(meta_input_dev);
	if (ret < 0) {
		input_free_device(meta_input_dev);
		goto ret_meta_input_unreg_device;
	}
	...
	return SUCCESS;
ret_meta_input_unreg_device:
	input_unregister_device(step_cnt_input_dev);
ret_step_cnt_input_unreg_device:
	input_unregister_device(uncalib_gyro_input_dev);
ret_uncalib_gyro_input_unreg_device:
	input_unregister_device(sig_motion_input_dev);
ret_sig_motion_input_unreg_device:
	input_unregister_device(uncal_mag_input_dev);
ret_uncal_mag_input_unreg_device:
	input_unregister_device(mag_input_dev);
ret_mag_input_unreg_device:
#ifdef CONFIG_SENSORS_SSP_SHTC1
	input_unregister_device(temp_humi_input_dev);
ret_tmep_humi_input_unreg_device:
#endif
	input_unregister_device(prox_input_dev);
ret_proximity_input_unreg_device:
	input_unregister_device(light_input_dev);
ret_light_input_unreg_device:
#ifdef CONFIG_SENSORS_SSP_UVIS25
	input_unregister_device(uv_input_dev);
#endif
	return ERROR;
#ifdef CONFIG_SENSORS_SSP_UVIS25
ret_uv_input_unreg_device:

	input_free_device(meta_input_dev);
#endif
ret_meta_input_free_device:
	input_free_device(step_cnt_input_dev);
ret_step_cnt_input_free_device:
	input_free_device(uncalib_gyro_input_dev);
ret_uncalib_gyro_input_free_device:
	input_free_device(sig_motion_input_dev);
ret_sig_motion_input_free_device:
	input_free_device(uncal_mag_input_dev);
ret_uncal_mag_input_free_device:
	input_free_device(mag_input_dev);
ret_mag_input_free_device:
#ifdef CONFIG_SENSORS_SSP_UVIS25
	input_free_device(uv_input_dev);
ret_uv_input_free_device:
#endif
	input_free_device(temp_humi_input_dev);
ret_temp_humidity_input_free_device:
	input_free_device(prox_input_dev);
ret_proximity_input_free_device:
	input_free_device(light_input_dev);
ret_light_input_free_device:
#ifdef CONFIG_SENSORS_SSP_BMP182
